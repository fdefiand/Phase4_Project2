class Camp < ApplicationRecord
  ...
  validate :check_for_camp_deactivation
  ...
  scope :empty, -> {joins(:registrations).group(:camp_id).having('count(*) = 0')}
  
    # instance methods
    
  def is_full?
    self.max_students <= enrollment
  end
  ...
  before_update :check_for_camp_deactivation
  
  
  before_destroy do
    check_for_students_registered
    camp_instructors.delete_all
  end
  ...
  def registration_for_camp_check?
    !self.registrations.length.zero?
  end
  
  def check_for_camp_deactivation
    return true if self.active
    registration_for_camp_check?

  end
  
  def check_for_students_registered
    registration_for_camp_check?
