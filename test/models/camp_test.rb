...
class CampTest < ActiveSupport::TestCase
  ...
  context "Within context" do
    ...
    should "return false when max students larger than number of registrations" do
      @camp1.max_students=5
      @camp1.save
      assert_equal @camp1.is_full?, false
    end
    
    should "return true when max students less than number of registrations" do
      @camp1.max_students=0
      @camp1.save
      assert_equal @camp1.is_full?, true
    end
    
    should "cancel destroy when there are students registered" do 
    ...
    should "remove instructors if camp is destroyed" do
      curriculum = FactoryBot.create(:curriculum, name: "Mastering Ruby")
      location = FactoryBot.create(:location, name: "Georgetown")
      camp = FactoryBot.create(:camp, curriculum: curriculum, location: location)
      camp_instructor = FactoryBot.create(:camp_instructor, camp: camp)
      camp.update(active: false)
      camp_instructor.camp.destroy 
      assert camp.destroyed?
      #p camp_instructor.instructor.destroyed?
    end
